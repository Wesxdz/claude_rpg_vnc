...
static rfbBool resize(rfbClient* client) {
	...
	int window_width = width > 0 ? width : 1024;
	int window_height = height > 0 ? height : 640;
	if(!sdlWindow) {
	...
	/* Don't use logical size - we'll handle scaling manually to support dynamic window resizing */
...
static void update(rfbClient* cl,int x,int y,int w,int h) {
	...
	if(quadrantView) {
		...
		for (int quad = 0; quad < 4; quad++) {
			...
			destRect.x = quad_offset_x + (quadrant_w - scaled_w) / 2;
			destRect.y = quad_offset_y + (quadrant_h - scaled_h) / 2;
			destRect.w = scaled_w;
			destRect.h = scaled_h;
...
static rfbBool handleSDLEvent(rfbClient *cl, SDL_Event *e)
{
	...
	case SDL_KEYUP:
	case SDL_KEYDOWN:
		if (e->key.keysym.sym == SDLK_F1 && e->type == SDL_KEYDOWN) {
			if (quadrantView) {
				...
				activeQuadrant = quad_y * 2 + quad_x;

				rfbClientLog("Entering interactive mode for quadrant %d\n", activeQuadrant);
			} else {
				rfbClientLog("Returning to quadrant view\n");
			}
